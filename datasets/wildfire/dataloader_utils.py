"""
Normalization statistics and feature utilities from WildfireSpreadTS.
(https://github.com/SebastianGer/WildfireSpreadTS, MIT License)
"""
...
def get_means_stds_missing_values(training_years: List[int]):
    """Return (means, stds, missing_values) arrays for the given pair of training years.

    Means/stds are pre-computed on unaugmented training data. Degree features and the
    categorical land cover type are NOT standardized (mean=0, std=1).
    """
    stats = {
        (2018, 2019): {
            'means': np.array([
    ...
    years_tuple = tuple(sorted(training_years))
    entry = stats[years_tuple]
    means = entry['means'].copy()
    stds = entry['stds'].copy()
    missing_values = entry['missing_values'].copy()

    # Do not standardize degree features or the categorical land cover type
...
def get_indices_of_degree_features():
    """Indices of features taking values in [0, 360] — transformed via sin."""
    return [7, 13, 19]
