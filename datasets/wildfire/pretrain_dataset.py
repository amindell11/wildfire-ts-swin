"""
PretrainDataset: reads single-day GeoTIFF files for spatial inpainting pre-training.

Each TIF has 19 raw bands. Preprocessing mirrors FireSpreadDataset to produce
40 normalized channels. Returns the full image as both input and target —
the masking is applied by the model.

Output per sample: (40, crop_side_length, crop_side_length)
"""
import glob
import os
import warnings
...
class PretrainDataset(Dataset):
    def __init__(
        ...
        # Filter out TIFs that are too small or have wrong band count
        valid_paths = []
        for p in self.tif_paths:
            try:
                with rasterio.open(p, 'r') as ds:
                    if (ds.count == 23
                            and ds.height >= crop_side_length
                            and ds.width >= crop_side_length):
                        valid_paths.append(p)
            except Exception:
                pass
        skipped = len(self.tif_paths) - len(valid_paths)
        if skipped > 0:
            warnings.warn(f"Skipped {skipped} TIFs (too small, wrong bands, or unreadable)")
        self.tif_paths = valid_paths
        print(f"PretrainDataset: {len(valid_paths)} valid TIFs "
              f"({skipped} skipped) x {crops_per_tif} crops = {len(valid_paths) * crops_per_tif} samples")
    ...
    def __getitem__(self, index):
        ...
        x = torch.from_numpy(img).unsqueeze(0)  # (1, 19, H, W)

        # NaN handling for active fire band
        ...
        # One-hot encode landcover -> 40 channels
